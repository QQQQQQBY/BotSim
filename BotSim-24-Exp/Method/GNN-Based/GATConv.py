...
class GAT(nn.Module):
    def __init__(self,value_num, text_num,feature_dim,classes,dropout,data,num_layers):
        ...
        for _ in range(num_layers):
            self.rgcn = GATConv(feature_dim,feature_dim,dropout=0.3)
            self.convs.append(self.rgcn)
        self.fc3 = nn.Linear(feature_dim,classes)


   
    def forward(self,value_feature, text_feature,edge_index,idx):
        ...
        for conv in self.convs:
            feature = conv(feature, edge_index=edge_index)     
...
def train(data, train_idx, val_idx, test_idx,model,num_epochs,lr,weight_decay,logger,model_file):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        output = model(value_feature, text_feature,data.edge_index, train_idx).float()        
        ...
        if loss_val < min_loss:
            ...
            torch.save(model.state_dict(),"/mnt/likun/qby/BotSim/BestModel/{}.pth".format(model_file))
...
def test(data,idx,model,loss_ce):
    ...
    output = model(value_feature, text_feature,data.edge_index, idx).float()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_dir', type=str, default='/mnt/likun/qby/BotSim/Log/rgcn_log.log', help='dir of logger')
    ...
    for i,seed in enumerate(args.random_seed):
        ...
        model = GAT(args.value_num, args.text_num,args.feature_dim,args.classes,args.dropout,dataloader,args.num_layers)
